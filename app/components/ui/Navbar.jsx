...
export const Navbar = () => {
    ...
    const navItems = [
        ...
        { 
            name: 'About us', 
            href: '/about',
            dropdown: ['Why Us?', 'Our Approach', 'Our Concern', 'Our Procedure']
        },
        { 
            name: 'Solutions', 
            href: '/solutions',
            dropdown: ['Research based Products', 'Ai Research']
        },
    ...
    return (
        <nav className={`fixed top-0 w-full z-50 transition-all duration-700 ${
            isScrolled 
                ...
                : 'bg-gradient-to-r from-slate-900/30 via-blue-900/25 to-indigo-900/30 backdrop-blur-md py-4'
        }`}>
                                    ...
                                    >
                                        ...
                                        {item.dropdown.map((dropdownItem, dropIndex) => (
                                            <a
                                                ...
                                                className="block px-6 py-3 text-sm font-medium text-gray-700 hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50 hover:text-blue-700 transition-all duration-200 mx-2 rounded-lg relative group/item"
                                            >
                                                <span className="relative z-10 flex items-center">
                                                    <span className="w-2 h-2 bg-gradient-to-r from-blue-500 to-indigo-500 rounded-full mr-3 opacity-0 group-hover/item:opacity-100 transition-opacity duration-200"></span>
                                                    {dropdownItem}
                                                </span>
                                            </a>
                                        ))}
                    ...
                    <div className="px-3 pt-4 pb-4 space-y-2 bg-white/98 backdrop-blur-lg mt-4 rounded-xl shadow-xl border border-gray-200/60">
                        {navItems.map((item, index) => (
                            <div key={item.name}>
                                <a
                                    href={item.dropdown ? '#' : item.href}
                                    className="block px-4 py-3 text-base font-medium text-gray-800 hover:text-blue-600 hover:bg-gradient-to-r hover:from-blue-50 hover:to-indigo-50 rounded-lg transition-all duration-300 relative"
                                    onClick={(e) => {
                                        if (item.dropdown) {
                                            e.preventDefault()
                                            setActiveDropdown(activeDropdown === index ? null : index)
